...
class Computer < Player
  ...
  #need to fix computer AI keeps choosing #4 if available...
  def find_move(board, marker)
    square = nil
    squares = board.squares
    Board::WINNING_LINES.each do |line|
      if squares.values_at(*line).count(marker) == 2
        square = line.select { |sq| board.unmarked_squares.include?(sq) }.first
      end
      break if !!square
    end
  ...
  def mark(board)
    square = nil
    square = find_move(board, MARKER) # offense
    square = find_move(board, Human::MARKER) if square.nil? # defense
